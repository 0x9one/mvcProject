...
class AbstractModel
{
    ...
    {
        $sql = 'DELETE UserId FROM ' . static::$tableName . ' JOIN app_users_profiles ON app_users.UserId = app_users_profiles.UserId WHERE  ' . static::$primaryKey . ' = ' . $this->{static::$primaryKey};
        $stmt = DatabaseHandler::factory()->prepare($sql);
        return $stmt->execute();
    }
